...
public class BaseDao {
  ...
  public BaseDao(
      @Qualifier("postgresExecutorService") ExecutorService executorService,
      @Qualifier("postgresExecutorServicePlatformThread")
          ExecutorService executorServicePlatformThread,
  ...
  public <T> CompletableFuture<List<T>> query(
      String sql, Object[] params, int[] types, RowMapper<T> rowMapper) {
    return queryDbWithSemaphore(() -> jdbcTemplate.query(sql, params, types, rowMapper));
  }

  public <T> CompletableFuture<List<T>> queryPlatformThread(
      String sql, Object[] params, int[] types, RowMapper<T> rowMapper) {
    return CompletableFuture.supplyAsync(
        () -> jdbcTemplate.query(sql, params, types, rowMapper), executorServicePlatformThread);
  }
